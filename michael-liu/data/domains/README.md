# 憨厚的麦总历年 5,344 篇推文 · 量化模型 领域分类开放数据集

> 本目录为 **憨厚的麦总（@Michael_Liu93）** 历年 5,344 篇推文基于 **量化模型 (实战系统)** 大模型评估生成的全景分类数据集，包含 7 大领域分卷，同时提供带 BOM 的 UTF-8-SIG CSV（Excel 零乱码直开）与标准 JSON 格式，完全开源免费，适合量化研究、回测与 AI Agent 知识库投喂。
...
- `jev_depth`: 量化模型 模型判定的专业技术与逻辑深度 (0.0 ~ 4.0)
- `jev_is_substantive`: 量化模型 模型判定的干货概率 (0.0 ~ 1.0)
- `jev_has_rule`: 量化模型 模型判定的具体交易/买卖实操规则判别 (0.0 ~ 1.0)
